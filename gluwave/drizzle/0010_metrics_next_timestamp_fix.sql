...
BEGIN
    ...
    WITH glucose_change AS (
        SELECT 
            ...
            LEAD(glucose.timestamp) OVER (PARTITION BY glucose.user_id ORDER BY glucose.timestamp DESC) AS timestamp_next,
            LAG(glucose.timestamp) OVER (PARTITION BY glucose.user_id ORDER BY glucose.timestamp DESC) AS timestamp_prev
        FROM glucose
    ...
    ORDER BY glucose_change.timestamp;
END;
$$ LANGUAGE plpgsql;